...
char const*const LED_BLINK = "/sys/devices/i2c-0/0-0032/led_blink";
char const*const LED_BLUE = "/sys/devices/i2c-0/0-0032/leds/B/brightness";
char const*const LED_GREEN = "/sys/devices/i2c-0/0-0032/leds/G/brightness";
char const*const LED_RED = "/sys/devices/i2c-0/0-0032/leds/R/brightness";
char const*const LED_PATTERN = "/sys/devices/i2c-0/0-0032/led_pattern";
...
static int write_leds(const struct led_config *led)
{
    ...
    is_solid = (led->delay_on==0 && led->delay_off==0) ? 1:0;

    if (!is_solid) {
        if ((count = snprintf(blink, sizeof(blink)-1, "0x%06x %d %d", led->color,
        ...
        blink[count]   = '\n';
        blink[count+1] = '\0';
        ALOGD("%s: color=0x%06x, delay_on=%d, delay_off=%d, blink=\"%s\".",
          __func__, led->color, led->delay_on, led->delay_off, blink);

    } else {
        /* Solid color, no blinking */
        red = (led->color >> 16) & 0xFF;
        green = (led->color >> 8) & 0xFF;
        blue = led->color & 0xFF;
    }
    ...
    if (!is_solid) {
        ...
        err |= write_int(LED_PATTERN, 0);
        err |= write_int(LED_RED, red);
        err |= write_int(LED_BLUE, blue);
        err |= write_int(LED_GREEN, green);
    }
    pthread_mutex_unlock(&g_lock);
